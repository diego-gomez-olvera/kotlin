...
class ReadWriteAccessCheckerDescriptorsImpl(private val project: Project) : ReadWriteAccessChecker {
    override fun readWriteAccessWithFullExpressionByResolve(assignment: KtBinaryExpression): Pair<ReferenceAccess, KtExpression> {
        val analysisSessionProvider = KtFe10AnalysisSessionProvider(project)
        val analysisSession = analysisSessionProvider.getAnalysisSession(assignment, KtReadActionConfinementLifetimeTokenFactory)
        val resolvedCall = with(analysisSession.analysisContext.analyze(assignment, Fe10AnalysisFacade.AnalysisMode.PARTIAL)) {
            assignment.getCall(this)?.getResolvedCall(this)
                ?: return ReferenceAccess.READ_WRITE to assignment
        }
